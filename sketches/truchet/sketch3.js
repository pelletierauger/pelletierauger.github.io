...
function showNumeral(position, x, y, tW, light, dark) {
    ...
    if (wavy) {
        ...
        switch (position) {
            case "A":
                showA(x - p, y + p, tileWidth + p2, light, dark);
                break;
            case "B":
                showB(x - p, y + p, tileWidth + p2, light, dark);
                break;
            case "C":
                showC(x - p, y + p, tileWidth + p2, light, dark);
                break;
            case "D":
                showD(x - p, y + p, tileWidth + p2, light, dark);
                break;
            case "E":
                showE(x - p, y + p, tileWidth + p2, light, dark);
                break;
            case "F":
                showF(x - p, y + p, tileWidth + p2, light, dark);
                break;
            default:
                showA(x - p, y + p, tileWidth + p2, light, dark);
        }
...
function showA(x, y, tW, light, dark) {
    ...
    for (var i = 0; i < linesDrawn; i++) {
        ...
        if (Math.abs(angle) != Math.PI &&
            Math.abs(angle) != Math.PI * 0.5 &&
            Math.abs(angle) != Math.PI * 1.5 &&
            Math.abs(angle) != Math.PI * 1.75 &&
            Math.abs(angle) != 0 &&
            Math.abs(angle) != Math.PI * 2 &&
            Math.abs(angle) != Math.PI * 1.25 &&
            Math.abs(angle) != Math.PI * 0.75 &&
            angle != Math.PI * 0.75 - Math.PI &&
            angle != Math.PI * 1.25 - Math.PI) {
            // if (Math.abs(aN - bN) > 10) {
            // if (angle == Math.PI * 0.75) {

            // if (Math.abs(angle) == Math.PI * 0.75) {
            // if ((angle) == Math.PI * 0.75 - Math.PI) {
            // if ((angle) == Math.PI * 1.25 - Math.PI) {
...
function showF(x, y, tW, light, dark) {
    ...
    for (var i = 0; i < linesDrawn * 1.5; i++) {
        var aN = Math.round(random(triangularArray.length - 1));
        var bN = Math.round(random(triangularArray.length - 1));
        var a = triangularArray[aN];
        var b = triangularArray[bN];
